...
import { Button, Card, Chip, ActivityIndicator, Portal, Modal } from 'react-native-paper';
...
export default function WriteScreen({ navigation, route }) {
  ...
  // Get prompt from navigation params if available
  const prompt = route?.params?.prompt;

  useEffect(() => {
    if (prompt) {
      setContent(`Prompt: ${prompt}\n\n`);
      // Focus on content input after a short delay
      setTimeout(() => {
          ...
          <Card style={styles.contentCard}>
            <Card.Content>
              <TextInput
                ...
                placeholder={prompt ? 'Continue writing...' : "What's on your mind?"}
                value={content}
