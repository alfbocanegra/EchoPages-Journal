      ...
      averageMood:
        monthEntries.length > 0
          ? monthEntries.reduce((sum, entry) => sum + (entry.mood?.intensity || 3), 0) /
            monthEntries.length
          : 0,
      streak: calculateWritingStreak(entries, new Date(year, month + 1, 0)),
    };

    return { calendarDays: days, monthStats: stats };
  }, [currentDate, entries]);

  const calculateWritingStreak = (entries: JournalEntry[], endDate: Date): number => {
    let streak = 0;
    ...
    >
      ...
      >
        ...
        >
          ...
          <div>🔥 {monthStats.streak} day streak</div>
      ...
      >
        ...
        <div>Click any date to view or create entries</div>
      </div>
    </div>
  );
};
