...
export class SyncService {
  ...
  public async getDeviceState(userId: string, deviceId: string): Promise<SyncDeviceState> {
    if (this.pgPool) {
      const { rows } = await this.pgPool.query(
  ...
  public async getChangesSinceVersion(
    userId: string,
    version: number,
  ...
  private async detectConflicts(changes: SyncChange[]): Promise<SyncConflict[]> {
    // Enhanced: auto-resolve simple conflicts, mark complex as manual
    ...
    for (const [key, entityChanges] of changesByEntity) {
      if (entityChanges.length > 1) {
        ...
        // If all fields except updatedAt/lastSyncAt are equal, auto-resolve (last-writer-wins)
        const ignoreFields = ['updatedAt', 'lastSyncAt', 'syncStatus'];
        const aData = { ...a.changeMetadata };
        const bData = { ...b.changeMetadata };
        ignoreFields.forEach(f => {
          delete aData[f];
          delete bData[f];
        });
        if (deepEqual(aData, bData)) {
          // Auto-resolve: keep the one with the latest updatedAt
          // No conflict record needed
          continue;
        }
        // For simple text fields, use last-writer-wins
        if (typeof aData.content === 'string' && typeof bData.content === 'string') {
          // Auto-resolve: keep the one with the latest updatedAt
          continue;
        }
        // Otherwise, mark as conflict and store both versions
        const conflict: Partial<SyncConflict> = {
          id: uuidv4(),
          changeId: a.id,
          conflictingChangeId: b.id,
  ...
  public async applyChanges(changes: SyncChange[], strategy: ResolutionStrategy): Promise<void> {
    for (const change of changes) {
      if (change.isConflict) continue; // Skip conflicted changes unless resolved
      try {
  ...
  private mapDeviceState(row: any): SyncDeviceState {
    return {
      ...
      deviceMetadata:
        typeof row.device_metadata === 'string'
  ...
  private mapSyncChange(row: any): SyncChange {
    return {
      ...
      changeMetadata:
        typeof row.change_metadata === 'string'
  ...
  public async getUnresolvedConflicts(userId: string, deviceId?: string): Promise<any[]> {
    ...
    if (this.sqliteDb) {
      return new Promise((resolve, reject) => {
        this.sqliteDb!.all(
          `SELECT * FROM sync_conflicts WHERE resolved = 0 AND user_id = ?`,
          [userId],
          (err: any, rows: any[]) => {
            if (err) reject(err);
            else resolve(rows);
  ...
  public async getConflictSummary(userId: string, deviceId?: string): Promise<any> {
    // Get unresolved (manual) conflicts
    const unresolved = await this.getUnresolvedConflicts(userId, deviceId);
    // Get recently resolved conflicts (last 24h, limit 10)
    let recentResolvedConflicts: any[] = [];
    let autoResolvedCount = 0;
    let manualResolvedCount = 0;
    const since = new Date(Date.now() - 24 * 60 * 60 * 1000); // last 24h
    if (this.pgPool) {
      const { rows } = await this.pgPool.query(
        `SELECT id, resolution_metadata, resolved_at, resolution_strategy
                 FROM sync_conflicts
                 WHERE resolved = true AND user_id = $1 AND resolved_at > $2
                 ORDER BY resolved_at DESC
                 LIMIT 10`,
        [userId, since]
      );
      recentResolvedConflicts = rows.map((r: any) => ({
        id: r.id,
        entityType: r.resolution_metadata?.local?.entityType || 'unknown',
        resolvedAt: r.resolved_at,
        strategy: r.resolution_strategy,
      }));
      const { rows: autoRows } = await this.pgPool.query(
        `SELECT COUNT(*) FROM sync_conflicts WHERE resolved = true AND user_id = $1 AND resolution_strategy = 'server_wins' AND resolved_at > $2`,
        [userId, since]
      );
      autoResolvedCount = parseInt(autoRows[0].count, 10);
      const { rows: manualRows } = await this.pgPool.query(
        `SELECT COUNT(*) FROM sync_conflicts WHERE resolved = true AND user_id = $1 AND resolution_strategy = 'manual' AND resolved_at > $2`,
        [userId, since]
      );
      manualResolvedCount = parseInt(manualRows[0].count, 10);
    }
    if (this.sqliteDb) {
      // SQLite version
      recentResolvedConflicts = await new Promise((resolve, reject) => {
        this.sqliteDb!.all(
          `SELECT id, resolution_metadata, resolved_at, resolution_strategy
                     FROM sync_conflicts
                     WHERE resolved = 1 AND user_id = ? AND resolved_at > ?
                     ORDER BY resolved_at DESC
                     LIMIT 10`,
      ...
      autoResolvedCount = await new Promise((resolve, reject) => {
        this.sqliteDb!.get(
          `SELECT COUNT(*) as count FROM sync_conflicts WHERE resolved = 1 AND user_id = ? AND resolution_strategy = 'server_wins' AND resolved_at > ?`,
          [userId, since.toISOString()],
          (err: any, row: any) => {
            if (err) reject(err);
      ...
      manualResolvedCount = await new Promise((resolve, reject) => {
        this.sqliteDb!.get(
          `SELECT COUNT(*) as count FROM sync_conflicts WHERE resolved = 1 AND user_id = ? AND resolution_strategy = 'manual' AND resolved_at > ?`,
          [userId, since.toISOString()],
          (err: any, row: any) => {
            if (err) reject(err);
            else resolve(row.count);
